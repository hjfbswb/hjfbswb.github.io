...
想象一个场景：UART 正在收发数据，CPU 正跑在 UART 的中断服务程序（ISR）里。这时 SysTick 到了——它是个普通中断，若优先级比正在跑的 ISR 高，会**抢断**正在执行的 ISR（这叫中断嵌套）。如果切换动作写在 SysTick 里，你就在"UART ISR 执行到一半"时换了栈。糟糕之处在于：UART ISR 的返回地址和中间状态都存在它自己的栈上，而你把 PSP 换成了另一个任务的栈——等 UART ISR 想返回时，它往栈上摸到的全是别人的数据。
