...
一个真实场景：固件 200K，Flash 512K 用得只剩 8K，网上搜到一招——编译加 `-ffunction-sections -fdata-sections`，链接加 `-Wl,--gc-sections`，让链接器裁掉没人用的代码。加上，重新构建：链接通过，bin 还真小了。烧录，上电——板子死了。一个灯都不闪。
...
- `. = ALIGN(4)` 把笔尖**抬**到下一个 4 字节边界（STM32 字长对齐，不对齐的地址硬件访问直接出错）；
- `_sdata = .` 在笔尖当前处贴一张门牌号。
