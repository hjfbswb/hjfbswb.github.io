...
顺便拆穿一个 PC 时代的直觉：在 Cortex-M4 上解引用 NULL 指针（读写地址 0）会怎样？**读**——不会崩，你会读到向量表的内容（地址 0 是合法的 Flash）；**写**——Flash 写保护触发 HardFault 异常。而 PC 上你熟悉的"段错误"，是操作系统 + MMU（内存管理单元）替你挡的，不是 CPU 天生会报错。MCU 上没人替你挡，错误的代价是"读到垃圾值悄悄算错"或"程序跑飞"。
