...
categories: [嵌入式]
last_modified_at: 2026-08-25 00:10:53 +0800
---
...
上一篇 [《Cortex-M4 与 MCU 开发》](/2026/08/23/cortex-m4-mcu-dev.html) 点过一盏灯：开时钟、配输出模式、往 ODR 写值。现在给它加个交互——一个按键按下时灯灭，松开时灯亮。假设 LED 接在 PD12，按键接在 PD1 和 GND 之间（开发板自带的用户按键在 PA0，这里为了只碰一个端口 GPIOD，用面包板随便接一根到 PD1 就行）：
...
2. **电平转换**：外部上拉接 3.3V，输出高就是 3.3V；接 5V，输出高就是 5V——引脚本身不用承受 5V，电平却能"翻译"过去。
...
"开漏（open-drain）"这个名字里就藏着历史。上世纪 TTL 时代，可编程 I/O 想共享一根线，用的是**集电极开路（open-collector）**——三极管的集电极裸露在外，同样"只能拉低、靠外部上拉弹回高位"。等到 CMOS 流行，"集电极开路"演化成了"漏极开路"（MOS 管的漏极），只换了个名字，思路一模一样。而 I2C 这个总线，从 1982 年发明至今，一直要求开漏——半个世纪前的设计决策，今天还钉在你每天用的每一块开发板上。
...
**先预测再看答案。** 编译之前先猜：两个函数各被翻译成几条指令？BSRR 版本真的比 ODR 版本少吗？

**答案**（ARM GCC，`-O2`；寄存器名随编译器版本略有出入）：

```
led_odr:                    ; 用 ODR：读-改-写，三条
    ldr  r3, =0x40020C14
    ldr  r2, [r3]           ; ① 读回旧值
    orr  r2, r2, #0x1000    ; ② 置位第 12 位
    str  r2, [r3]           ; ③ 写回去

led_bsrr:                   ; 用 BSRR：一条，完事
    ldr  r3, =0x40020C18
    str  r2, [r3]           ; 一次内存写
```

ODR 版本是"读-改-写"三条指令，中间那一下 `orr` 和最后的 `str` 之间，**任何中断都能插进来**；BSRR 版本是一条 `str`，一次写，谁也插不进脚。这就是 L2 说"BSRR 天然原子"的铁证——你亲眼看到了编译器把它们编成了什么。
