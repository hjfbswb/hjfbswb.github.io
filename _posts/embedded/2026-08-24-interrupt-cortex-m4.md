...
上一篇 [《Cortex-M4 与 MCU 开发》](/2026/08/23/cortex-m4-mcu-dev.html) 讲过 CPU 的三种"等"法：轮询、中断、DMA。中断是硬件"敲门"通知 CPU，不用空等。现在我们把门敲起来。最简单也最常见的敲门人是 **SysTick**——Cortex-M4 内核自带的 24 位倒计时定时器，也是《FreeRTOS 核心原理》里那个每 1ms 敲一次节的拍来源。用它学中断，没有外设配置的干扰。
...
（`GPIOD_ODR` 那几个宏的定义，和上一篇点灯代码一模一样。）
